...
**24 个内置工具**覆盖基础场景：文件操作（读 / 写 / 列目录 / 追加 / 移动 / 复制 / 删除 / 建目录）、Shell、HTTP（get / post / 任意请求 / 抓网页 / 下载）、时间与 JSON 工具、记忆，以及 `notify`。全部在沙箱管控下执行——文件走路径白名单、Shell 走命令白名单、HTTP **读写分治**（读：默认放行 + SSRF 黑名单；写：域名通配符白名单）——每次调用都有审计。`notify` 按名字推送到通知渠道（飞书 / 企业微信 / 钉钉 / 通用 webhook），渠道本身也是支持完整 CRUD 的动态注册表。
