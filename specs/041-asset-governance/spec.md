...
- 团队 HTTP API（#546）：`oryxos.web.teams-api.enabled`（默认关→404）；`/api/v1/teams` + `/api/v1/users/{u}/teams`；RBAC 映射 `MANAGE_MEMBERS`；无 Admin UI
...
- orgs 层级 / JIT 建队 / Admin 管队 UI（#535+#539+#546：成员表 + 目录表 + CLI + teams HTTP API，默认关）
