...
- 运行时：`ToolExecutor` 在 `PrincipalContext` 有主体时对 Agent 做 `decide(RUN_AGENT)`（#527）；无上下文（CLI）不裁决。入站 webhook 已认证时把主体装入 `PrincipalContext`，并由 `AgentExecutionService.triggerAsync` 传到后台线程（#529）。接口与值对象在 `oryxos-core`，不反向依赖 `oryxos-web`。
