...
/**
 * 按 Provider 注册表参数手工构造 {@link TextEmbedder}（FR-007：复用同一套凭证，不另建配置）。复用与 ChatModel 完全相同的 OpenAI SDK
 * 构建链（normalizeOpenAiBaseUrl / 超时，research D2）；mock 名走内置确定性向量。
 *
 * <p>embedding provider 未配置时不静默回退（DeepSeek 等无 embedding 端点，「取第一个可用项」会静默拿到错误 provider）——调用方拿到可读异常后按
...
public class ProviderEmbeddingModelFactory {
  ...
  public TextEmbedder buildOne(String name, String apiKey, String baseUrl, String model) {
    ...
    String base = ProviderChatModelFactory.normalizeOpenAiBaseUrl(baseUrl);
    Duration connectTimeout = ProviderChatModelFactory.connectTimeout();
    Duration readTimeout = ProviderChatModelFactory.readTimeout();
