...
### Added
- 业务流程与工单 Agent 标杆方案：审批 HITL Flow、失败补偿与时间线回放、评测集与离线验收脚本（#479，Epic #460）。
- 企业知识与支持 Agent 标杆方案：样例工作区、引用策略、权限样例、转人工 Flow、评测集与离线验收脚本（#478，Epic #460）。
- 可解释模型路由与成本策略：按难度/敏感度驻留/预算/时延与定价信号重排 Agent 候选，决策与 fallback 原因可查询；默认关可灰度回滚（051，#477）。
- 任务级成本账本、预算与配额：按 run/Agent/团队/模型归因 token/工具费/时延，可查询对账；默认关（050，#476）。
- 集群下 Agent/Skill/Knowledge 共享内容版本源：快照、共享 active 指针、原子切换与回滚（049，#473；027 共享半边之上）。
- Agent/Skill/Prompt/Flow 离线评测 harness、基线对比与默认关闭的回归门禁（048，#472）。
