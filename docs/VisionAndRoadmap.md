...
| 子 Agent 委托 / A2A | ⚠️ delegate_agent + Agent Card + `message/send` + `message/stream`；跨节点续刀 | C |
...
| **任务发布 + 多 Agent 自组织交付（Agent 团队）** | ⚠️ MVP+并行+JPA+有界重规划+本地 A2A send/stream；跨节点续刀 | **I** |
