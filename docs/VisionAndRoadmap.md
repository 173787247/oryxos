...
| 子 Agent 委托 / A2A | ⚠️ delegate_agent + Card/send/stream + 出站 `a2a_send`（host 白名单）；多跳/鉴权续刀 | C |
...
| **任务发布 + 多 Agent 自组织交付（Agent 团队）** | ⚠️ MVP+并行+JPA+有界重规划+本地 A2A + 出站 `a2a_send`；团队跨节点续刀 | **I** |
